...
def parse_args():
    ...
    parser.add_argument('--patch_size', type=int, default=2, help='Patch size for ViT (default: 4)')
    ...
    parser.add_argument('--fm_steps', type=int, default=50, help='Denose step for flow matching')
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (images, labels) in progress_bar:
            ...
            t = torch.randint(0, args.fm_steps, (images.shape[0],), device=device)
            t_as_interpolate = (t.float() / args.fm_steps).view(-1, 1, 1, 1) # 把帧数广播到 (B, 1, 1, 1) 的形状
            x_t = noise * t_as_interpolate + images * (1 - t_as_interpolate)
            
            pred_vector = model(x_t, t, labels)
            loss = F.l1_loss(images - noise, pred_vector)
